...
class TetrisGame:
    ...
    def update_board(self, rot_idx, x):
        ...
        """
        piece_type, rotations = self.current_piece
        # Guard against invalid rotation indices (can happen if an agent ignores masking)
        if rot_idx < 0 or rot_idx >= len(rotations):
            raise ValueError(
                f"Invalid move attempted: rotation={rot_idx} out of range for piece={piece_type}"
            )
        piece = rotations[rot_idx]
    ...
    def render(self, valid_actions, placement_mask=None, pre_clear_board=None, return_fig=False):
        ...
        """
        # First clear output in notebooks for interactive display, but skip this
        # when exporting frames (return_fig=True) to avoid terminal control noise.
        if not return_fig:
            try:
